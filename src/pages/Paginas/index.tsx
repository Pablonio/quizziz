...
export default function Paginas() {
    ...
    if (isRegistering) {
      ...
      try {
        const response = await axios.post('/api/inicio-sesionUsuario/auth', {
          identificador: formData.correo, // Aquí enviamos 'correo' para iniciar sesión
        ...
        router.push('/Examen'); // Redirigir al dashboard u otra página
      } catch (error) {
  ...
  return (
    <div className='mt-10 sm:mx-auto sm:w-full sm:max-w-sm flex min-h-full flex-col justify-center px-6 py-8 lg:px-8 block w-full rounded-md border-0 py-1.5 bg-white	'>
      ...
      <form onSubmit={handleSubmit} method="POST" className='space-y-6'>
        ...
        <div className='flex justify-center'>
          <button type="submit" className='text-center text-black bg-indigo-300 p-2 py-2 rounded-lg'>{isRegistering ? 'Registro' : 'Login'}</button>
        </div>
      </form>
      <button onClick={toggleMode} className='text-center text-black underline'>
        {isRegistering ? 'Cambiar a Inicio de Sesión' : 'Cambiar a Registro'}
      </button>
      <Toaster />
    </div>
  );
